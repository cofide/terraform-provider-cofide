...
func (r *Resource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_connect_aws_agent_core_discovery_config"
}
...
// upsert sets the cloud account's agent_core_discovery_config to exactly
// what's in the plan, using an update mask so no other field on the cloud
// account is touched.
func (r *Resource) upsert(ctx context.Context, plan Model) (Model, diag.Diagnostics) {
	agentCoreConfig, diags := modelToProto(ctx, plan)
	if diags.HasError() {
		return Model{}, diags
	}
	...
	cloudAccount := &cloudaccountpb.CloudAccount{
		...
		Provider: &cloudaccountpb.CloudAccount_Aws{
			Aws: &cloudaccountpb.AWSAccount{AgentCoreDiscoveryConfig: agentCoreConfig},
		},
	}
	updateMask := &cloudaccountsvcpb.UpdateCloudAccountRequest_UpdateMask{AwsAgentCoreDiscoveryConfig: true}

	updateResp, err := r.client.CloudAccountV1Alpha1().UpdateCloudAccount(ctx, cloudAccount, updateMask)
	if err != nil {
		diags.AddError(
			"Error setting AWS AgentCore discovery config",
			fmt.Sprintf("Could not update cloud account %q: %s", cloudAccountID, err),
		)
		return Model{}, diags
	}

	return protoToModel(cloudAccountID, updateResp.GetAws().GetAgentCoreDiscoveryConfig()), diags
}
...
func (r *Resource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError(
			"Error reading AWS AgentCore discovery config",
	...
	agentCoreConfig := cloudAccount.GetAws().GetAgentCoreDiscoveryConfig()
	if agentCoreConfig == nil {
	...
	newState := protoToModel(cloudAccountID, agentCoreConfig)
	resp.Diagnostics.Append(resp.State.Set(ctx, &newState)...)
}
...
func (r *Resource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	cloudAccount := &cloudaccountpb.CloudAccount{
		...
		Provider: &cloudaccountpb.CloudAccount_Aws{
			Aws: &cloudaccountpb.AWSAccount{AgentCoreDiscoveryConfig: nil},
		},
	}
	updateMask := &cloudaccountsvcpb.UpdateCloudAccountRequest_UpdateMask{AwsAgentCoreDiscoveryConfig: true}
	...
	if err != nil && status.Code(err) != codes.NotFound {
		resp.Diagnostics.AddError(
			"Error deleting AWS AgentCore discovery config",
			err.Error(),
		)
