...
- Provider config can be supplied via HCL attributes. `api_token` and `insecure_skip_verify` also fall back to environment variables (`COFIDE_API_TOKEN`, `COFIDE_INSECURE_SKIP_VERIFY`), as does the deprecated `connect_url` (`COFIDE_CONNECT_URL`). `connect_tls_grpc_target` and `connect_tls_grpc_server_name` are HCL-only by design — see the deprecation commit for rationale.
